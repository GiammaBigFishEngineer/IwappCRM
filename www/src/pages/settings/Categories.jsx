...
export const Categories = () => {
  ...
  const list = RenderList(URL+'categorie');
  ...
  function Delete(id) {
    ...
    if (answer) {
        ...
        window.location.href = URL+'elimina_categoria?id='+id;
    }   
  }
  ...
  <tbody>
    {list.map(item=>(
      ...
      <td> <span style={{background:item.colore}} className="badge rounded-pill">{item.colore}</span></td>

      <td>
      {organizer ? 
      <>
       <a className="text-primary" data-bs-toggle="modal" data-bs-target={'#'+item.id}>
      ...
      <td>
      {organizer ? 
        <a href="#" onClick={(e)=>{Delete(item.id)}}>
